...
const Dock = ({ className, children, magnification = DEFAULT_MAGNIFICATION, distance = DEFAULT_DISTANCE }: DockProps) => {
  ...
  // On touch devices, disable magnification by using BASE_SIZE (no scaling effect)
  const effectiveMagnification = isTouchDevice ? BASE_SIZE : magnification;
  const effectiveDistance = isTouchDevice ? 0 : distance;

  return (
    <DockContext.Provider value={{ mouseX, magnification: effectiveMagnification, distance: effectiveDistance }}>
      <motion.div
